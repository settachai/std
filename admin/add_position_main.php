...
$str="select * from position where position_name='$_POST[position_name]' or position_detail='$_POST[position_detail]' ;";
		$res=mysql_query($str);
		$rows=mysql_num_rows($res);
		if($rows==1){	
echo "<span class='style15'>ตำแหน่งนี้มีแล้วครับ ไม่สามารถเพิ่มซ้ำได้</span> <br > <br ><img src='../images/progress_dot_line.gif' />";
echo "<script>setTimeout(\"location='add_position.php'\",500);</script>";		
}else{
$sql = " insert into position ( position_name, position_detail) VALUES ( '$_POST[position_name]', '$_POST[position_detail]');";
mysql_query("set names tis620"); // ทำให้อ่านภาษาไทยได้
$result = mysql_query($sql);  // ประมาณคำสั่ง sql ที่มาจาก $sql
if($result)
{
echo "<span class='style15'> กำลังเพิ่มตำแหน่งพนักงาน</span> <br > <br >โปรดรอสักครู่<br ><img src='../images/loading42.gif' />";
echo "<script>setTimeout(\"location='view_position.php'\",500);</script>";
}else{
echo "<span class='style15'> ไม่สามารถเพิ่มตำแหน่งเรียบร้อยแล้ว</span> <br > <br >โปรดรอสักครู่<br ><img src='../images/progress_dot_line.gif' />";
  echo "<script>setTimeout(\"location='add_position.php'\",1000);</script>";
}
}
								?>
							<br ><br ><br >  <br ></td>					
